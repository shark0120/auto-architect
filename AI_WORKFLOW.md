...
## 10. 標準授權與使用者指令速查 (Standing Authorization & Commands)

### 10-A. 使用者標準授權(2026-07-20 由使用者明示設立)

> 凡依本檔 §2 循環完成、且 §3 閘門(G1/G2/G3,動模板加 G4)**全綠**的變更,
> AI **可自主 commit 並 push 到 `origin/main`,無需逐次再向使用者確認**。
>
> 例外(仍需使用者逐次同意):§5 的 P4 對外發佈類 — npm publish、GitHub Release、
> 宣傳貼文發佈、任何動用帳號/金錢/新平台的行為;以及任何刪除遠端分支/歷史的操作。
>
> 撤銷方式:使用者刪除本段即撤銷。閘門紅著就 push = 違反授權。

### 10-B. 使用者指令速查 — 想叫 AI 自主做事,開新 session 打這些

| 你想要 | 打這個 | 會發生什麼 |
|---|---|---|
| **自主推進到底** | `/start` 或 `開跑` 或 `自己跑到底,不用問我` | autopilot:讀本檔 §5 認領任務 → 實作 → 閘門 → 上傳 → 自動下一輪,直到卡住或做完 |
| **限時燒額度** | `/ai 4h`(或 `燒 4 小時`) | 小時制戰役:排時程、平行偵察、無限迭代、每小時壓縮存檔可跨 session 續跑;打 `戰役進度` 查進度 |
| **品質總攻** | `/max` | 高強度多代理掃全專案,把剩餘額度轉成經驗證的品質改進(高耗) |
| **掛機定時跑** | `/loop 30m /start` | 每 30 分鐘自動喚醒跑一輪 autopilot(本機掛著即可) |
| **單任務重火力** | 訊息開頭加 `ultracode`,可再加 `+500k` | 該則訊息啟用多代理 workflow 編排 + token 預算目標(例:`ultracode +500k 完成 P0-1~P0-4 並上傳`) |
| **睡前一句話** | `我去睡了,照 AI_WORKFLOW 自主迭代,做完就上傳` | 白話也能觸發 autopilot,綜合以上 |

**換了機器、沒有這些 skill 時**照樣能自主 — 打這句萬用指令即可:
> 「讀 repo 裡的 AI_WORKFLOW.md,照 §2 循環自主迭代,每輪閘門全綠就用 scripts/upload.ps1 上傳,不用問我。」

### 10-C. 免人值守的前置條件(缺一就會半路卡住)

1. **首推完成過一次**(人做):憑證快取後,之後所有 session push 全自動。
2. **權限模式**:掛機前把 Claude Code 權限調成允許編輯/執行(或加白名單),否則半夜停在確認彈窗等你。
3. **雲端排程注意**:`/schedule` 的雲端 agent 拿不到這台機器快取的 GitHub 憑證;要雲端推送需另外設 PAT。**掛機建議用本機 `/loop`。**
